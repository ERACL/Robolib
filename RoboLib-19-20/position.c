...
void initPosition(bool isBlueSide);
...
void initPosition(bool isBlueSide) {
  ...
  if (isBlueSide) {
    __position.x = c->initialX_BlueSide / c->mmPerEncode;
    __position.y = c->initialY_BlueSide / c->mmPerEncode;
    __position.orientation = c->initialOrientation_BlueSide * PI / 180;
  }
  else {
    __position.x = c->initialX_YellowSide / c->mmPerEncode;
    __position.y = c->initialY_YellowSide / c->mmPerEncode;
    __position.orientation = c->initialOrientation_YellowSide * PI / 180;
  }
